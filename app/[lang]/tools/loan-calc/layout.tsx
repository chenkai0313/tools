...
export async function generateMetadata({ params }: { params: Promise<{ lang: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    keywords: 'loan calculator, mortgage calculator, amortization, equal principal, equal interest, 贷款计算器, 房贷计算器, 等额本息, 等额本金, 还款计算, 月供计算',
      ...
      description: lang === 'zh'
        ? '在线贷款计算器，支持等额本息和等额本金两种还款方式。输入贷款金额、年利率、期限，输出每月还款明细和累计利息。'
        : 'Online loan calculator with amortization schedule. Supp...',
    },
    alternates: {
      languages: { 'zh': '/zh/tools/loan-calc', 'en': '/en/tools/loan-calc' },
      canonical: `https://schg.xyz/${lang}/tools/loan-calc`,
    },
  }
}
